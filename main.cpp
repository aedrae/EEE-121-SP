...
// --- second dataset --- //
    ...
    // 1D convolution
    vector<int> conv1d(ds1.size() + ds2.size() - 1, 0); // output size is 5
    vector<float> ds2_flip;

    for (int i = 1; i <= 3; i++){
        ds2_flip.push_back(ds2[ds2.size() - i]); // flips h
    }

    for (int i = 0; i < conv1d.size(); i++) {
        for (int j = 0; j < ds2_flip.size(); j++) {
            if (i - j >= 0 && i - j < ds1.size()) { //checks if index i - j is a valid index into vector x
                conv1d[i] += ds1[i - j] * ds2_flip[j]; // At output position i, add the product of each filter value h[j] and the aligned input value x[i - j], if it's within bounds.
            }
        }
    }
...
// 2D implementation
    ...
    */
    ...
    // CROSS CONVOLUTION STEP
    
    // input rows or rowsize1 = height
    //input columns or columnsize1 = width

    int outputRows = rowsize1 - rowsize2 + 1;
    int outputCols = columnsize1 - columnsize2 + 1;
    ...
    for (int i = 0; i < outputRows; i++) {
        for (int j = 0; j < outputCols; j++) {
            int sum = 0;
            for (int m = 0; m < rowsize2; m++) {
                for (int n = 0; n < columnsize1; n++) {
                    sum += ds1[i + m][j + n] * ds2[m][n];
                }
            }
            output[i][j] = sum;
        }
